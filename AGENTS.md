...
├── plan.md                    # 全体設計と Phase 計画 (Single Source of Truth)
├── QA_skill_invocation.md     # Skill 起動方法の設計議論記録
...
│   ├── XXX_template.md
│   └── 001_*.md, 002_*.md, ...
├── docs/                      # ★ EDINET 公式仕様の置き場（設計ドキュメントではない）
│   └── edinet/                # ESE140206.pdf 等の公式仕様書（gitignore 済）
└── manual_test/               # 手動テスト用 (Skill コピー、history.md 等。gitignore 済)
```

**サブモジュールは無い。** 過去の類似プロジェクトの慣習で `docs/` をルートに置いているが、本プロジェクトでは **EDINET 公式仕様 PDF/XLSX の置き場**であり設計ドキュメントではない。
...
| ファイル | 責務 |
|---------|------|
| `plan.md` | **本プロジェクトの SSoT**。全体設計、ディレクトリ構成、キャッシュポリシー、Phase 計画と完了条件、リスク管理 |
| `QA_skill_invocation.md` | Skill 起動方法（`${CLAUDE_SKILL_DIR}` 絶対パス方式）の設計議論と最終決定 |
...
Skill 内のスクリプトは **Claude Code Agent Skills 公式慣習**に従い `${CLAUDE_SKILL_DIR}` 絶対パスで起動する。`python -m scripts.X` ではなく `python3 ${CLAUDE_SKILL_DIR}/scripts/X.py` を使う。詳細は [QA_skill_invocation.md](QA_skill_invocation.md) と [SKILL.md](skills/japan-stock-analysis/SKILL.md) §4 を参照。
...
`report_*.html` / `data_*.json` などの分析成果物は **Claude 実行時の CWD**（ユーザーの作業ディレクトリ）に出力する。Skill 配下に出力してはならない（plan §3.4 / [BUG-001](bugs/001_output_lands_in_skill_dir_not_cwd.md)）。
...
- **`plan.md` を Single Source of Truth とする**: 設計判断は `plan.md` を最上位とし、各種 docs は plan を補強・詳細化する関係。`plan.md` と他 docs に矛盾が見つかったら `plan.md` を正として判断、必要なら ↓ で許可を取って修正する
- **`plan.md` の変更には開発者の許可が必要**: 設計の根幹に影響するため、変更前に必ず開発者に確認を取ること
